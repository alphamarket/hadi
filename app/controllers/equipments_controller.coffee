...
class EquipmentsController extends BaseController
  ...
  list_action: (args) ->
    $.response_success db.select 'equipments'
  ...
  delete_action: (data) ->
    $.response_success db.delete 'equipments', (item) -> item.id.toString() is data.id.toString()

  create_action: (data) ->
    # check for limited version
    return $.response_fail global.limited_version_error_msg if global.limited_version and db.count('equipments') >= limited_version_limit
    ...
    data = db.insert 'equipments', data
    # convert persian date to date object
